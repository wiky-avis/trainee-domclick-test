...
## Клиентам:
Клиент кликнув по ссылке "Клиентам", попадает на струницу заполения формы заявки, где может указать свой телеграмм ID, чтобы получать уведомления. Поле этого заявка сохранятся в базе данных и видна только сотрудникам должность которых соответствуют типу заявки.
